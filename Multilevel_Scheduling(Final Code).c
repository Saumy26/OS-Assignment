...
int pos,q,temp1,sum=0,wt2[20],tt2[20];
...
void priority_schedule()
{
    ...
    for(i=0;i<y;i++)
    {
    	tt2[i]=bt_2[i]+wt2[i];
        sum = sum+tt2[i];
        printf("\nP[%d]\t\t\t%d\t\t\t %d\t\t\t %d\n",p2[i],bt_2[i],wt2[i],tt2[i]);
    }
    avg_turnaround2 = sum/y;
    printf("\nAverage Waiting time  :: \t%f", avg_waiting2);
    printf("\nAverage Turnaround time  :: \t%f\n", avg_turnaround2);
    ...
    for(i=0;i<y;i++)
    {
    	while(bt_2[i]!=0)
    	{
    		if(bt_2[i]>10)
    		{
				printf("\nP[%d] of Q2 is running for 10 units",i+1);
				bt_2[i]=bt_2[i]-10;
			}
			else if(bt_2[i]<=10)
			{
				printf("\nP[%d] of Q2 is running for %d units",i+1,bt_2[i]);
				bt_2[i]=0;
			}
		}
	}
...
void rr1_schedule()
{
	printf("Time Quantum between the queues is : 10\n");
	for(i=1;i<total;i=i+10)
	{
		...
		if(t2>10)
		{
			printf("Q2 is running for 10 units\n");
			t2=t2-10;
		}
		else if(t2<=10 && t2!=0)
		{
			printf("Q2 is running for %d units\n",t2);
			t2=0;
		}
		if(t3>10)
		{
			printf("Q3 is running for 10 units\n");
			t3=t3-10;
		}
		else if(t3<=10&&t3!=0)
		{
			printf("Q3 is running for %d units\n",t3);
			t3=0;
		}
	}
}
